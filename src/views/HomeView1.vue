...
const init = () => {
  ...
  camera = new THREE.PerspectiveCamera(75, testBox.value.offsetWidth / testBox.value.offsetHeight, 0.1, 1000)
  //摄像机向z轴移动五个单位 以便能看到3d效果 (默认摄像机和物体的坐标都在原点)
  camera.position.z = 5
  // 4. 创建渲染器，并设置画布大小，添加到 DOM 显示  antialias设置为true后 盒子的边框就没有锯齿状了
  renderer = new THREE.WebGLRenderer({ antialias: true })
  // renderer.setSize(window.innerWidth, window.innerHeight)
  renderer.setSize(testBox.value.offsetWidth, testBox.value.offsetHeight)

  // renderer.setSize(100, 100)
  // console.log(testBox.value.offsetWidth,testBox.value.offsetHeight)  //获取dom的宽高
  // 把画面添加到页面上
...
const resizeRender = () => {
  window.addEventListener('resize', () => {
    renderer.setSize(testBox.value.offsetWidth, testBox.value.offsetHeight) //场景的宽高
    camera.aspect = testBox.value.offsetWidth / testBox.value.offsetHeight //摄像机的宽高
