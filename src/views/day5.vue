...
onMounted(() => {
  ...
  css2dRenderer = obj.css2dRenderer
...
const createSprite = () => {
  ...
  // 使用：
  // 1. 准备纹理对象并加载图片素材
  const map = new THREE.TextureLoader().load('image/pkq.jpg')
...
const create2d = () => {
  ...
  const object2d = new CSS2DObject(div) //把css标签转换成2d
  // console.log(object2d)
  object2d.position.set(0, 2, 0)
  scene.add(object2d)
}
...
const resizeRender = () => {
  window.addEventListener('resize', () => {
    ...
    css2dRenderer.setSize(testBox.value.clientWidth, testBox.value.clientHeight) //32渲染的宽高
